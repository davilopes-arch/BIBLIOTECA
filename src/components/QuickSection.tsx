  ...
  return (
    <div className="space-y-4 mb-6">
      ...
      {favoriteItems.length > 0 && (
        <div>
          <div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 mb-2">
            <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500" />
            <span>Seus Favoritos ({favoriteItems.length})</span>
          </div>

          <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-thin scrollbar-thumb-neutral-300 dark:scrollbar-thumb-neutral-700">
            {favoriteItems.map(({ category, tutorial }) => (
              <div
                key={tutorial.id}
                onClick={() => onSelectTutorial(category, tutorial)}
                className="flex-shrink-0 w-64 p-3 rounded-xl bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 hover:border-neutral-400 dark:hover:border-neutral-600 transition-all cursor-pointer shadow-xs"
                style={{
                  borderLeftWidth: '3.5px',
                  borderLeftColor: category.cor || '#FF5A1F'
                }}
              >
                <p className="text-[10px] font-bold uppercase tracking-wider text-neutral-400 truncate mb-0.5">
                  {category.nome}
                </p>
                <h4 className="font-semibold text-xs text-neutral-900 dark:text-neutral-100 line-clamp-1 mb-1.5">
                  {tutorial.titulo}
                </h4>
      ...
      {popularItems.length > 0 && (
        <div>
          <div className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wider text-neutral-600 dark:text-neutral-400 mb-2">
            ...
            <span>Mais Acessados pela Equipe</span>
          </div>
