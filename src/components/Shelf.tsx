...
export const Shelf: React.FC<ShelfProps> = ({
  ...
  return (
    <div
      ...
      className={`rounded-xl border bg-white dark:bg-neutral-900 transition-all duration-150 shadow-2xs overflow-hidden ${
    ...
    >
      ...
      <div
        ...
        className="group flex items-center justify-between px-3.5 py-2.5 cursor-pointer select-none hover:bg-neutral-50 dark:hover:bg-neutral-800/40 transition-colors"
      >
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          {/* Color Indicator Dot */}
          <span
            className="w-3 h-3 rounded-full shrink-0 shadow-2xs ring-2 ring-white dark:ring-neutral-900"
            style={{ backgroundColor: category.cor || '#FF5A1F' }}
          />

          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h2 className="font-bold text-sm sm:text-base text-neutral-900 dark:text-neutral-100 truncate">
                {category.nome}
              </h2>
              {isEditMode && !canEditCategory && (
                <span className="text-[9.5px] uppercase font-semibold tracking-wider text-neutral-400 bg-neutral-100 dark:bg-neutral-800 px-1.5 py-0.5 rounded">
                  Somente Leitura
                </span>
              )}
            </div>
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <span className="text-[11px] font-mono text-neutral-500 bg-neutral-100 dark:bg-neutral-800 px-2 py-0.5 rounded-md">
            {filteredTutorials.length} {filteredTutorials.length === 1 ? 'item' : 'itens'}
          </span>
              ...
              {canManageCategoryStructure && (
                <>
                  ...
                  <button
                    ...
                    className="p-1 rounded-md text-neutral-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-950/40 transition-colors cursor-pointer"
      ...
      {/* Expandable Body */}
      {isOpen && (
        <div className="p-3 pt-1 border-t border-neutral-100 dark:border-neutral-800/80 bg-neutral-50/40 dark:bg-neutral-900/30">
          {/* Empty State */}
          {filteredTutorials.length === 0 ? (
            <div className="py-6 flex flex-col items-center justify-center text-center text-neutral-400 gap-1.5">
              <Inbox className="w-6 h-6 opacity-50" />
              <p className="text-xs font-medium">Nenhum tutorial encontrado nesta categoria.</p>
              {isShelfEditable && (
                <button
                  onClick={onAddTutorial}
                  className="mt-0.5 text-xs font-semibold text-orange-600 hover:underline cursor-pointer"
                >
